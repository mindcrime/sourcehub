...
class DbRealm 
{
	...
	def authenticate(authToken) {
		...
		println "Found user '${user.userId}' in DB"
		...
		def account = new SimpleAccount(user, "", "NeddickLoginRealm");
	...
	def hasRole(principal, roleName)
	{
		User.withTransaction
		{
			...
			Role role = user.roles.find { it.name.equals( roleName ) };
	...
	{
		println "checking for requiredPermission: ${requiredPermission}";
		...
		User.withTransaction
		{
			...
			retval = permissions?.find { permString ->
				...
				// permission.
				println "found permString: ${permString}";
